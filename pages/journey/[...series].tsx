...
export default function Tags() {
  ...
  const { data, error } = useSWR(
    `${process.env.NEXT_PUBLIC_APP_BASEURL}/articleseriesget?seriesId=${router?.query?.series?.[1]}`,
    fetcher
  );

  console.log(data, error, " subcategory file fetcher");
      ...
      <section className={`${cx.blogSection}`}>
        <Container>
          {data && data?.status ? (
            <Row>
              {data &&
                data?.result?.titleId?.map((article: any, index: number) => {
